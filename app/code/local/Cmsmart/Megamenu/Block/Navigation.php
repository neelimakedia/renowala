...
class Cmsmart_Megamenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
   ...
   protected function _renderCategoryMenuItemHtml($category, $level = 0, $isLast = false, $isFirst = false,
        $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if($level == 1){    		
         $html[] = '<a style="background-color:'. $this->backgroundcolor($megamenu).' border-bottom: 2px solid '.$this->fontcolor($megamenu).'" class="catagory-level1" href="'.$this->getCategoryUrl($category).'"'.$linkClass.'>';        
			...
			$currentCategoryImage = Mage::getBaseUrl('media').'catalog/category/' . $imgaes;
			$onmouseover = ' onmouseover="document.getElementById(\'category_menu_image_display\').src=\'' . $currentCategoryImage . '\'"';
			$html[] = '<a  style="background-color:'. $this->backgroundcolor($megamenu).'" href="'.$this->getCategoryUrl($category).'"' .$linkClass . $onmouseover . '>';
        }
        if($showthumbnail == 1){
			if ($imgaes) {
        ...
        if ($childrenWrapClass) {
            ...
            $html[] = $this->getBlockRight($megamenu);  
              ...
              if($megamenu[0]['category_children']==0)   {   
                if($htmlChildren) {
                    if($level == 0){
						$imgaeCategory =  Mage::getModel('catalog/category')->load($category->getId())->getThumbnail();
						$imageHtml = '<img id="category_menu_image_display" src="' . Mage::getBaseUrl('media').'catalog/category/' .$imgaeCategory .'" width="400px" height="350px" style="float: left;z-index: 1" />';
						$htmlChildren .= '<li class="category_menu_image">' . $imageHtml . '</li>';
    ...
    public function widthp($idc){
		...
		$imageWidth = $widthp * 0.4;
		$widthp = $widthp - $imageWidth;
        return $widthp;       
